...
import Model.Home;
import Model.LoginPopup;
import Model.Pip;
import Model.QuestionManagerFX;
...
public class CommandController implements ColorParser, InputValidator, IndexOffset {
	...
	public void runCommand(String text, boolean isPlayerInput) {
		...
		if (command.equals("/move")) {
			...
			runCheatCommand();
		}else if (command.equals("/qu_manage")){
			openQuestionsManagement();
		} else if (command.equals("/game_history")){
			openGameRecordManagement();
		/*
	...
	private void openQuestionsManagement() {
		infoPnl.print("opening questions management...", MessageType.ANNOUNCEMENT);
		new LoginPopup().start(new Stage());
//		new QuestionManagerFX().start(new Stage());
		
	}
	
	// my modify 
	private void openGameRecordManagement() {
		infoPnl.print("opening GameRecord management...", MessageType.ANNOUNCEMENT);
//		new LoginPopup().start(new Stage());
		new gameRecordsManagerFX().start(new Stage());
...
//		int too = Integer.parseInt(to);
		if(RandomPip.RandomPipS.equals(to)) {
			runNext();
			runNext();
		}
...
//	////////////////
	private void runNext() {
		if (gameplay.isStarted()) {
			...
			infoPnl.print("Game not started. Enter \"/start\" to start the Model.", MessageType.ERROR);
		}
	}
...
//				///////////////////
				...
				if (!RandomPip.mood.equals("easy")) {
					boolean result = false;
//					System.out.println(RandomPip.isFirst);
					while(!result) {
						if(RandomPip.isFirst){
							RandomPip.isFirst  = false;
							result = true;
							continue;
						}
						QuizApp quizApp = new QuizApp();
//						System.out.println(RandomPip.qDiff);
							result = quizApp.startQuiz(new Stage(),String.valueOf(RandomPip.qDiff));
...
//				///////////////////////////////
	...
	/**
	 * Command: /start
	 * Enters into gameplay mode, i.e. start Model.
	 */
	private void runStartCommand() {
		if (!gameplay.isStarted()) {
			infoPnl.print("Starting Model...", MessageType.ANNOUNCEMENT);
			root.restartGame();
		} else {
			...
			if (!gameplay.isRolled()) {
				if (!gameplay.isMaxDoubling()) {
					if (!root.isCrawfordGame()) {
						...
						infoPnl.print("Unable to propose doubling the stakes, in the middle of a Crawford Model.", MessageType.ERROR);
					}
				} else {
	...
	private void runDoubleAcceptCommand() {
		...
		if (gameplay.isDoubling()) {
			infoPnl.print("Doubling cube accepted, Model continues.", MessageType.ANNOUNCEMENT);
	...
	/**
	 * Command: /save
	 * Saves Model log (text on info panel) to text file.
	 */
	...
	/**
	 * Command: /restart
	 * Starts a new instance of the Model.
	 */
	...
	private void runTestCommand() {
		if (!gameplay.isStarted()) {
			...
			infoPnl.print("Cannot test when Model is started.", MessageType.ERROR);
		}
	}
