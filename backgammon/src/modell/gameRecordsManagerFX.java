...
public class gameRecordsManagerFX extends Application {
     ...
     * Loads and displays all game records without filtering by difficulty.
     */
    @Override
    public void start(Stage primaryStage) {
        primaryStage.setTitle("Game Records App");

        // Load all game records from the JSON file
        loadGameRecordsFromFile("src/game/gameRecords.json");

        // If there are no records, show an alert and exit
        if (gameRecords.isEmpty()) {
            showAlert("No Records",
                      "No game records found in the JSON file.",
                      null, null);
            return;
        }
    ...
    /**
     * Loads game records from the specified JSON file into the `gameRecords` list.
     */
    private void loadGameRecordsFromFile(String filePath) {
        try {
            ...
            showAlert("File Error",
                      "Error loading game records: " + e.getMessage(),
                      null, null);
        }
    }
